...
// ---------------------------------------------------------------------------
// Versioning — bump this whenever ExplorerState shape changes in a breaking way.
// Old persisted data with a different version is discarded and replaced with
// initialState so the app never starts with a shape it doesn't understand.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Shape validators — every field is checked before the state is trusted.
// These run on the raw unknown value coming out of JSON.parse so TypeScript
// cannot help us; explicit runtime checks are the only guard.
// ---------------------------------------------------------------------------
...
function isValidFileNode(v: unknown): v is FileNode {
  ...
  if (v.type === 'folder') {
    ...
    if (Array.isArray(v.children) && !v.children.every(isValidFileNode)) return false;
  }
...
function isValidState(v: unknown): v is ExplorerState {
  ...
  if (v.selectedNodeId !== null && typeof v.selectedNodeId !== 'string') return false;
  if (!Array.isArray(v.expandedFolders)) return false;
  if (!v.expandedFolders.every((id: unknown) => typeof id === 'string')) return false;
  if (v.openedFileId !== null && typeof v.openedFileId !== 'string') return false;
  return true;
}
...
export function loadState(): ExplorerState {
  ...
  try {
    ...
    if (!isValidState(candidate)) {
      console.warn('[storage] Persisted state failed validation — using default.');
      return defaultState;
    }
